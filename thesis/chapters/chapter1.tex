...
Предыдущие две версии были основаны на параллелизации классической версии Беллмана-Форда. В основе следующего алгоритма лежит идея обхода в ширину (Алгоритм 2). В качестве основы для параллельной версии такого алгоритма был взят параллельный обход в ширину, предложенный Умутом Акаром и Майком Рэйни. А именно в качестве ключевой структуры данных используется Frontier, который представляет из себя набор всех ребер, исходящих из некоторого множества вершин. И эта структура данных поддерживает разделение, слияния и итерирование по ребрам. Подробнее о внутреннем устройстве их подхода речь зайдет во 2 части работы в контексте решения задачи поиска расстояний между каждой парой вершин. Пока лишь приведем псевдокод 
...
\State Рекурсивно делим Frontier пополам, атомарно рекаксируем ребра и строим новый Frontier
